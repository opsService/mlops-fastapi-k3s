...
# 환경 변수 로드
POSTGRES_HOST = os.getenv("POSTGRES_SERVICE_HOST", "postgresql-service")
POSTGRES_PORT = os.getenv("POSTGRES_SERVICE_PORT", "5432")
POSTGRES_DB = os.getenv("POSTGRES_DB", "mlflow_db")
POSTGRES_USER = os.getenv("POSTGRES_USER") # Secret에서 로드됨
POSTGRES_PASSWORD = os.getenv("POSTGRES_PASSWORD") # Secret에서 로드됨

MINIO_ENDPOINT = os.getenv("MLFLOW_S3_ENDPOINT_URL", "http://minio-service:9000").replace("http://", "") # MinIO endpoint (http:// 제거)
MINIO_ACCESS_KEY = os.getenv("AWS_ACCESS_KEY_ID") # Secret에서 로드됨
MINIO_SECRET_KEY = os.getenv("AWS_SECRET_ACCESS_KEY") # Secret에서 로드됨
...
async def health_check() -> tp.Dict:
    ...
    try:
        ...
        health_status["dependencies"]["postgresql"] = {"status": "failed", "error": str(e)}
    ...
    try:
        ...
        minio_host_port = MINIO_ENDPOINT.split(':')
        ...
        if not minio_client.bucket_exists("mlflow-artifacts"):
            raise Exception("MLflow artifacts bucket does not exist or is not accessible.")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{MLFLOW_TRACKING_URI}/health", timeout=5) # MLflow는 보통 /health 엔드포인트 제공
        ...
        health_status["dependencies"]["mlflow_tracking"] = {"status": "failed", "error": str(e)}
