...
@router.post("/predict")
async def predict(task_id: str, request: PredictRequest):
    ...
    try:
        ...
        user_id = request.userId  # Get userId directly from the request body
        logger.info(f"Forwarding prediction for task {task_id} (user: {user_id}) to {endpoint}")

        async with httpx.AsyncClient() as client:
            # Send only the pure data (request.data) to the inference server
            response = await client.post(f"{endpoint}/predict", json=request.data, timeout=30)
