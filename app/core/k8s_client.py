...
class K8sClient:
    def __init__(self):
        ...
        if os.getenv("KUBERNETES_SERVICE_HOST"):
            ...
            try:
                ...
                logger.error("kubeconfig를 로드할 수 없습니다. Kubernetes 컨텍스트가 올바르게 설정되었는지 확인하세요.")
        ...
        logger.info(f"Kubernetes 클라이언트가 네임스페이스 '{self.namespace}'에 대해 초기화되었습니다.")
    ...
    def create_train_job(self,
        ...
        try:
            ...
            logger.error(f"Kubernetes Job {job_name} 생성 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Job 생성: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes Job {job_name} 생성 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def get_job_status(self, job_name: str):
        ...
        try:
            ...
            if e.status == 404:
                logger.warning(f"네임스페이스 {self.namespace}에서 Job {job_name}을 찾을 수 없습니다.")
                return None
            logger.error(f"Job {job_name} 상태 조회 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Job 상태 조회: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Job {job_name} 상태 조회 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def delete_job(self, job_name: str):
        ...
        try:
            ...
            if e.status == 404:
                logger.warning(f"네임스페이스 {self.namespace}에서 삭제할 Job {job_name}을 찾을 수 없습니다.")
                return {"message": f"삭제할 Job {job_name}을 찾을 수 없습니다."}
            logger.error(f"Kubernetes Job {job_name} 삭제 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Job 삭제: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes Job {job_name} 삭제 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def get_pods_for_job(self, job_name: str) -> List[client.V1Pod]:
        ...
        try:
            ...
            logger.error(f"Job {job_name}의 Pod 조회 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def get_pod_logs(self, pod_name: str, tail_lines: Optional[int] = None) -> str:
        ...
        try:
            ...
            if e.status == 404:
                logger.warning(f"네임스페이스 {self.namespace}에서 로그를 가져올 Pod {pod_name}을 찾을 수 없습니다.")
            ...
            logger.error(f"Pod {pod_name} 로그 가져오기 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def create_inference_deployment(self,
        ...
        try:
            ...
            logger.error(f"Kubernetes Deployment {deployment_name} 생성 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Deployment 생성: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes Deployment {deployment_name} 생성 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def create_inference_service(self, service_name: str, deployment_name: str, port: int = 8000):
        logger.info(f"Deployment {deployment_name}을 위한 Kubernetes 서비스 {service_name} 생성 시도")
        ...
        try:
            ...
            logger.error(f"Kubernetes 서비스 {service_name} 생성 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 서비스 생성: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes 서비스 {service_name} 생성 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def create_inference_ingress(self, ingress_name: str, service_name: str, host: str, path: str, service_port: int = 8000):
        logger.info(f"서비스 {service_name}을 위한 Kubernetes Ingress {ingress_name} 생성 시도")
        ...
        try:
            ...
            logger.error(f"Kubernetes Ingress {ingress_name} 생성 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Ingress 생성: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes Ingress {ingress_name} 생성 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def delete_deployment(self, deployment_name: str):
        ...
        try:
            ...
            if e.status == 404:
                logger.warning(f"네임스페이스 {self.namespace}에서 삭제할 Deployment {deployment_name}을 찾을 수 없습니다.")
                return {"message": f"삭제할 Deployment {deployment_name}을 찾을 수 없습니다."}
            logger.error(f"Kubernetes Deployment {deployment_name} 삭제 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Deployment 삭제: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes Deployment {deployment_name} 삭제 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def delete_service(self, service_name: str):
        ...
        try:
            ...
            if e.status == 404:
                logger.warning(f"네임스페이스 {self.namespace}에서 삭제할 서비스 {service_name}을 찾을 수 없습니다.")
                return {"message": f"삭제할 서비스 {service_name}을 찾을 수 없습니다."}
            logger.error(f"Kubernetes 서비스 {service_name} 삭제 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 서비스 삭제: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes 서비스 {service_name} 삭제 중 예상치 못한 오류: {e}", exc_info=True)
    ...
    def delete_ingress(self, ingress_name: str):
        ...
        try:
            ...
            if e.status == 404:
                logger.warning(f"네임스페이스 {self.namespace}에서 삭제할 Ingress {ingress_name}을 찾을 수 없습니다.")
                return {"message": f"삭제할 Ingress {ingress_name}을 찾을 수 없습니다."}
            logger.error(f"Kubernetes Ingress {ingress_name} 삭제 오류: {e.body}", exc_info=True)
            raise Exception(f"K8s API 오류 Ingress 삭제: {e.reason} - {e.body}")
        except Exception as e:
            logger.error(f"Kubernetes Ingress {ingress_name} 삭제 중 예상치 못한 오류: {e}", exc_info=True)
